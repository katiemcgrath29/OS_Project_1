...
def initialise(pageNames, frameData, usePattern, pagesInRAM, numRefs, numFrames):
	...
	random.seed()
	pstart=random.randint(1, 800) 
	pend=pstart + 99
	while i < numFrames:			#while number of page names generated < frames available
		pName=random.randint(pstart, pend)		#generate a random number as page name
	...
	for clock in range(numRefs):
		...
		if pageNames[page] not in pagesInRAM:	#if it has not already been installed in RAM
			...
			frameData[pageIndex][refIndex] = 1 	            #set its reference bit

		pageIndex=pagesInRAM.index(pageNames[page])		#Get its index in pagesInRAM
		if clock<numRefs-5:						#if this is not a later reference
			frameData[pageIndex][refIndex] = 0	#clear reference bit (for older referenced pages)
#			print(clock,pageNames[page],0," not recent")
		else:                                   #if this is a recent reference
			frameData[pageIndex][refIndex] = 1 	#set its reference bit		ref = 1 
#			print(clock,pageNames[page],1," recent")			
...
#		order = "oldest first"
	...
	print("\nFrames in", group, snum, "memory are shown in frame number order.")
#	print("\nFrames in", group, snum, "memory are shown in", order, "order.")
	print("\nFrames in", group, snum, dt, "memory before victim replacement were:")
...
#	data.append([pageIn,clock,1,0,1,1])	#replace victim page with incoming page with ref=1 and modify=0
	data[head]=[pageIn,clock,1,0,1,1]
...
#	return oldData, candidates, newUsePattern, vFrame	#return original data, possible candidate list& full use pattern of references, 
	...
	return oldData, candidates, usePattern, vFrame	#return original data, possible candidate list& full use pattern of references, 
												#and candidate victim frame numbers
...
#find LRU page in the use pattern that is ordered with least recent reference last
	...
	referenceOrder.reverse()			#sort reference history into 'least recent first'
...
#	data.sort(key=getSecond)			#put the RAM data back in order of page arrival
	data[vFrame[0]]=[pageIn,clock,1,0,1,1]		
...
#initialise all reference counters to 0
	...
	print("First we will show the", type, "history")
	my_input("Press <ENTER> to continue to the history")
	...
	for r in range(len(refPattern)):	#for every period of n references (n == period (or less at the end))
		...
		my_input("Press <ENTER> to continue to next part of the history")
...
#find the victim page in the data as the page with the lowest reference counter
	...
	newData[vFrame[0]]=[pageIn,clock,1,0,1,1] #new bit HERE	
...
#find the victim page in the data as the page with the lowest reference counter
	...
	newData.sort(key=getSecond)				#put data back in oldest page first order
	vFrame=[newData.index(victim)]				#record victim's frame number
	newData[vFrame[0]]=[pageIn,clock,1,0,1,128] #new bit HERE
#	newData.remove(victim)					#remove it from memory
...
pageIn = 999		#page to bring in to full RAM
...
numRefs = 34		#number of initial memory references to generate at start
numFrames = 13		#number of page frames in RAM
...
while (not group.isalnum()):
    group=removePunc(my_input("Please enter your class group (e.g. SD2A): "))
snum=""
...
print("NOTE\n")
print("\tPlease include a snip of this ID number,\n")
print("\tgroup name and timestamp in your submitted document.\n")
print("\tAnd, when they appear, also please include later snips \n")
print("\tof the ID number, group name and timestamp in your submitted document.\n\n\n\n")
my_input("Press <ENTER> to continue")
...
while ans!=0:
	...
	if ans==3:	#LRU
		...
		sure="n"
		sure=my_input("\nThis remove all the old data. Are you sure? y/[n]")
		if sure=="y":
			pageNames = []		#list of pages that may enter RAM at start
			pageIn = 998		#page to bring in to full RAM
			frameData = []		#data on pages in RAM as [page, arrival time, reference bit, modify bit, reference counter]
			usePattern = []		#list of references to pages so far
			pagesInRAM = []		#list of pages that have entered RAM at start
			numRefs = 34		#number of initial memory references to generate at start
